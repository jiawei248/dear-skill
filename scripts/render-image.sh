...
# Source .env to ensure API keys are available in the environment.
# In cron contexts, env vars from .env may not be exported automatically.
...
def get_realism_booster(genre):
    """Return a lightweight anti-AI-tell instruction. Keep it short — the creative prompt does the heavy lifting."""
    return (
        "Make this look like it was made by a talented human, not generated by AI. "
        "Avoid: plastic/uniform textures, shadowless lighting, objects fusing into surfaces, fake text/barcodes, "
        "glowing particles/golden dust, everything emitting light, HDR over-saturation, concept-art rendering style. "
        "The image should be beautiful and striking with a specific aesthetic point of view. "
    )
...
def build_prompt(brief):
    ...
    # Prepend realism booster based on genre
    realism = get_realism_booster(genre)
    parts = [realism, primary] if realism else [primary]
    if genre:
...
fallback_models = image.get("fallback_models") or []
...
try_model_with_fallback() {
  ...
  # Create a modified runtime JSON with the specified model
  local modified_json
  modified_json="$(python3 - "$runtime_json" "$model" <<'PY'
import json
import sys
runtime = json.loads(sys.argv[1])
runtime["model"] = sys.argv[2]
print(json.dumps(runtime, ensure_ascii=True))
PY
)"

  local detection_json
  detection_json="$(detect_provider_path "$modified_json")"
  ...
  local result
  result="$(dispatch_provider_path "$modified_json" "$(json_get "$detection_json" "provider_path")")"
  local render_mode
  render_mode="$(json_get "$result" "render_mode")"
  
  if [ "$render_mode" = "image_urls" ]; then
    echo "$result"
  else
    echo "FAIL"
  fi
...
PRIMARY_MODEL="$(json_get "$RUNTIME_JSON" "model")"
RESULT="$(try_model_with_fallback "$RUNTIME_JSON" "$PRIMARY_MODEL")"
...
# Try fallback models
FALLBACK_MODELS="$(json_get "$RUNTIME_JSON" "fallback_models")"
if [ -n "$FALLBACK_MODELS" ] && [ "$FALLBACK_MODELS" != "[]" ]; then
  FALLBACK_LIST="$(python3 - "$FALLBACK_MODELS" <<'PY'
import json
import sys
models = json.loads(sys.argv[1]) if sys.argv[1] else []
for m in models:
    print(str(m).strip())
PY
)"
  ...
  while IFS= read -r fallback_model; do
    ...
    RESULT="$(try_model_with_fallback "$RUNTIME_JSON" "$fallback_model")"
    if [ "$RESULT" != "FAIL" ]; then
      echo "$RESULT"
      exit 0
    fi
...
# All models failed — fall back to H5
...
print_result \
  ...
  "$(json_get "$RUNTIME_JSON" "genre")" \
  "all_models_failed" \
  "Tried $PRIMARY_MODEL and fallback models, all failed"
